...
const AccountRegistration = () => {
    ...
    const validPages = useSelector((state: RootState) => state.registrationValidations) as RegistrationValidationPages;
    ...
    const handleGoToNextPage = () => {
        currentPage + 1 > amountOfPages ? setCurrentPage(amountOfPages) : setCurrentPage(currentPage + 1);
    }
    ...
    const getRequestData = (): RequestData => {
        return {
            ...
            endpoint: currentRole === Roles.GUEST
            ...
            <FormControl variant="standard" className="data-container-main">
                <div className="data-container-head-row">
                    {currentRole === Roles.GUEST ?
                        ...
                        : <h2 className="data-container-head-row-h2 ">{t('registration.register')}</h2>}

                    {currentPage !== (1 as number) &&
                        <div className="data-container-head-row-rest" onClick={handleGoToPreviousPage}>
                            <BackArrow className="data-container-head-row-rest-arrow"/>
                            <label className="data-container-head-row-rest-label">{t('containers.back')}</label>
                        </div>}
                </div>
                <ProgressBar progress={progress}/>
                {currentPage === 1 && <RegistrationFirstPage/>}
                {(currentPage === 2 && validPages.firstPage) && <RegistrationSecondPage/>}
                {(currentPage !== 2 && validPages.firstPage && validPages.secondPage && currentPage === amountOfPages) &&
                    <RegistrationThirdPage/>}
                {((currentPage === 1 && validPages.firstPage)
                        || (currentPage === 2 && validPages.firstPage && validPages.secondPage)
                        || (currentPage === 3 && validPages.firstPage && validPages.secondPage && validPages.thirdPage)) &&
                    <Box className=" data-container-buttons">
                        <BasicButton
                            content={(currentPage === amountOfPages &&
                                ((amountOfPages === (2 as number) && validPages.firstPage && validPages.secondPage)
                                    || amountOfPages === 3 && validPages.firstPage && validPages.secondPage && validPages.thirdPage))
                                ? t('buttons.submit') : t('buttons.next')}
                            onClick={currentPage < amountOfPages ? handleGoToNextPage : async () => handleParentIsOpenState(true)}
                        />
                    </Box>
                }
