...
import { updateBasicRegistrationData } from "../../redux/actions/registrationActions";
import {
  validateCompanyName,
...
const RegistrationFirstPage = () => {
  ...
  const validFirstPage = useSelector(
  ...
  const [validUsername, setValidUsername] = useState<boolean>(
    validateUsername(registrationData.username as string),
  );
  const [validPassword, setValidPassword] = useState<boolean>(
    validatePassword(registrationData.password as string),
  );
  ...
  const [validEmail, setValidEmail] = useState<boolean>(
    validateEmail(registrationData.email as string),
  );
  const [validCompanyName, setValidCompanyName] = useState<boolean>(
    validateCompanyName(registrationData.companyName as string),
  );
