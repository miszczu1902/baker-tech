...
const RegistrationSecondPage = () => {
  ...
  const validSecondPage = useSelector(
  ...
  const [validFirstname, setValidFirstname] = useState<boolean>(
    validateFirstname(registrationData?.personalData?.firstname as string),
  );
  const [validLastname, setValidLastname] = useState<boolean>(
    validateLastname(registrationData?.personalData?.lastname as string),
  );
  const [validPhoneNumber, setValidPhoneNumber] = useState<boolean>(
    validatePhoneNumber(registrationData?.personalData?.phoneNumber as string),
  );
  const [validNip, setValidNip] = useState<boolean>(
    validateNipNumber(registrationData?.billingDetails?.nip as number),
  );
  const [validRegon, setValidRegon] = useState<boolean>(
    validateRegonNumber(registrationData?.billingDetails?.regon as number),
  );
  ...
  const handlePhoneNumberChange = (
    ...
    dispatch(
      updatePersonalDataDuringRegistration({ phoneNumber: phoneNumber }),
    );
  };
  ...
  useEffect(() => {
    ...
    dispatch(setSecondPageRegistrationStatus(page));
