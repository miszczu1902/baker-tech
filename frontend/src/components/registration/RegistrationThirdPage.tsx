...
import {validateCity, validatePostalCode, validateStreet, validateStreetNumber} from "../../utils/regexpValidator";
...
const RegistrationThirdPage = () => {
    ...
    const validThirdPage = useSelector((state: RootState) => state.registrationValidations) as RegistrationValidationPages;
    const [validStreet, setValidStreet] = useState<boolean>(validateStreet(registrationData.address?.street as string));
    const [validStreetNumber, setValidStreetNumber] = useState<boolean>(validateStreetNumber(registrationData.address?.streetNumber as string));
    const [validPostalCode, setValidPostalCode] = useState<boolean>(validatePostalCode(registrationData.address?.postalCode as string));
    const [validCity, setValidCity] = useState<boolean>(validateCity(registrationData.address?.city as string));
    ...
    useEffect(() => {
        const page = {
            ...
            thirdPage: validStreet && validStreetNumber && validPostalCode && validCity
        }
