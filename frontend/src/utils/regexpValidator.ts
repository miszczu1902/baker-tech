...
export const validatePhoneNumber = (phoneNumber: string): boolean => {
  return (
    /^(?:\+\d{1,3}\s?)?(?:\d{3,4})?\d{6,10}$/.test(phoneNumber) &&
...
export const validateRegonNumber = (regon: string | number): boolean => {
  return (
    /^(?:\d{9}|\d{14}|\d{18})$/.test(regon as string) &&
    _.size(regon.toString()) > 0
  );
};
...
export const validateDeviceDataField = (content: string): boolean => {
  return !_.isEmpty(content) && _.size(content) <= 100;
};

export const validateSerialNumber = (content: string): boolean => {
  return !_.isEmpty(content) && _.size(content) <= 200;
};

export const validatePositiveValue = (content: number): boolean => {
  return content > 0;
};

export const validateDescription = (content: string): boolean => {
  return !_.isEmpty(content) && _.size(content) <= 2000;
};
